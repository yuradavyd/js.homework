...
fetch('https://jsonplaceholder.typicode.com/users')
    ...
    .then((users) => {
        ...
        for (const user of users) {
            ...
            wrap.innerHTML = `<div class="card-header">id: ${user.id}</div> <div class="card-body"><h5 class="card-title">name: ${user.name}</h5></div> `;
            let btn = document.createElement('button');
            btn.addEventListener('click', () => {
                window.location.assign(`./user-details.html${"#"+"userDetails/"+user.id}`);
            })
            btn.innerText = 'click';
